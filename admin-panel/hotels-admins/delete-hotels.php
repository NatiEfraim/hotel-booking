...
if (isset($_GET['id'])) {
    ...
    try {
        // Fetch the image data from the 'hotels' table
        $getImage = $conn->prepare("SELECT * FROM hotels WHERE id = :id");
        ...
        if ($fetch) {
            $imageFilePath = "hotel-images/" . $fetch->image;
            ...
            $deleteQuery = $conn->prepare("DELETE FROM hotels WHERE id = :id");
            $deleteQuery->bindValue(':id', $id, PDO::PARAM_INT);
            $deleteQuery->execute();

            // Hotel deleted successfully
            echo "<script>alert('The hotel has been deleted')</script>";
